...
/*****************************************************************************************
PHASE 1A: Server-C boot up, and be ready to listen the request
===The code for initializing, binding socket, and building network address has referenced
====================="http://www.beej.us/guide/bgc/"======================================
*****************************************************************************************/
	int socket_serverc, socket_send; //Variable for building udp socket
	...
	if((socket_serverc=socket(AF_INET, SOCK_DGRAM, 0))<0){
	...
	aws_addr.sin_port = htons(UDP_SERVER_C);

	sin_size=sizeof(aws_addr);

	//bind the socket and the address-----Beej's
	if((bind(socket_serverc,(struct sockaddr*)&aws_addr,sizeof(aws_addr))) == -1){
	...
	/*---------------------------------------------------
	 * Building the dictionary according to "backendC.txt"
	 *---------------------------------------------------*/
    fstream fin;
    fin.open("backendC.txt");
	...
	cout<<"The ServerC is up and running using UDP on port <"<<UDP_SERVER_C<<">."<<endl;
...
*****************************************************************************************/
    while(1){
    		...
    		if((pktLen=recvfrom(socket_serverc, aws_pkt, BUFF_SIZE, 0, (struct sockaddr*)&aws_addr, &sin_size))==-1){
    		...
    		cout<<"The ServerC received input <"<<aws_data[1]<<"> and operation <"<<aws_data[0]<<">."<<endl;
    		...
    		//if the operation is prefix
    			...
    			//printOut the results message on the screen
    			cout<<"The ServerC has found <"<<to_string(def_data.size())<<"> matches"<<endl;
    		 ...
    		 ********************/
        		...
        		//send all the resutls to the aws
        		for(int i=0; i<def_data.size(); i++){
        			pktLen=sendto(socket_send, def_data[i].c_str(), BUFF_SIZE, 0, (struct sockaddr*)&send_addr, sizeof(send_addr));
